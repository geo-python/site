...
# Read data
dataFrame = pd.read_csv(input_fp, sep=',', skiprows=8)
...
temp_row8 = dataFrame.loc[8, 'TEMP']
print(temp_row8)
...
# Here, we see that now we have a subset of our DataFrame with only rows where the ``TEMP_Celsius`` is above 15 and the dates in ``YEARMODA`` column start from 15th of June. 
